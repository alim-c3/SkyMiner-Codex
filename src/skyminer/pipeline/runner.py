...
def run_pipeline(
    ...
    *,
    ...
    if detect:
        candidates = _detect_and_score(cfg, lightcurves)
    ...
    if report and candidates:
        ...
        generate_reports(cfg, candidates, top_k=top_k)
...
def _detect_and_score(cfg: SkyMinerConfig, lightcurves: list[LightCurve]) -> list[Candidate]:
